...
def get_grads(model, data_loaders, 
          num_batches, logger, 
          args
    ...
    for i, b_data in enumerate(biggest_dl):
        current_iter = model.module.current_iter
        input_dicts.clear()
        ...
        if (i % args.print_freq == 0 or i == (biggest_size - 1)) and get_rank() == 0:
            ...
            if not args.is_retrain:
                ...
                for dset in datasets:
                    logger.log_text(f"{dset}: \n {model.module.task_gating_params[dset]}")
                
                logger.log_text(f"{model.module.policys}")
...
def evaluate(model, data_loaders, data_cats, logger, num_classes):
    ...
    for dataset, taskloader in data_loaders.items():
        ...
        task_gate = torch.zeros(model.module.task_gating_params[dataset].size()).float().cuda()
        
        
        num_used_blocks = []
        
        minimum_count = sum(model.module.num_per_block)
        maximum_count = 0
        mac_count = 0.
        ...
        for i, data in enumerate(taskloader):
            ...
            task_gate += model.module.policys[dataset]
            # num_used_blocks += torch.sum(model.module.policys[dataset][:, 0])
            num_used_blocks.append(torch.sum(model.module.policys[dataset][:, 0]))
            mac_count += macs
        ...
        logger.log_text(
            f"all counting: {int(torch.sum(sum_list))}/{(i+1) * get_world_size() * sum(model.module.num_per_block)} --> averaged counting: {round(float(averaged_num_used_blocks), 2)}")
