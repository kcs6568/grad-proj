...
class MTANStatic(nn.Module):
    def __init__(self,
        ...
        self.is_global_attetion = [True, True, True, False]
        self.global_attention = nn.ModuleList([
            self.global_att_layer(attention_ch[idx], attention_ch[idx] // 2) for idx in range(len(attention_ch)-1)])
    ...
    def forward(self, data_dict, hyp):
        images = {k: img_label[0] for k, img_label in data_dict.items()}
