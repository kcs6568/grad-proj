...
class LossCalculator:
    ...
    def balancing_loss_for_gating(self, output_losses):
        ...
        for data in self.data_cats:
            ...
            data_loss *= self.task_weights[data]
...
def training(model, optimizer, data_loaders, 
    ...
    is_gate_opt_seperatly = True if isinstance(optimizer, dict) > 1 else False
    ...
    if is_gate_opt_seperatly or is_gate_opt_unified:
...
def evaluate(model, data_loaders, data_cats, logger, num_classes, block_num, effect_type, effect_direction):
    ...
    for dataset, taskloader in data_loaders.items():
        ...
        task_kwargs = {"task_list": {dataset: task}, "block_num": block_num,
                       "effect_type": effect_type, "effect_direction": effect_direction}
    ...
    logger.log_text(model.module.policys)
