    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch == args.gate_epoch and epoch > 0:
            ...
            if 'multi' in args.lr_scheduler:
                print("Previous Milestones:", lr_scheduler.milestones)
                logger.log_text("Previous Milestones:", lr_scheduler.milestones)
                
                from collections import Counter
                lr_scheduler.milestones = Counter(args.lr_steps)
                
                logger.log_text("Reconstructed Milestones:", lr_scheduler.milestones)
                print("Reconstructed Milestones:", lr_scheduler.milestones)
                
            logger.log_text("Reconstructed Optimizer:", optimizer)
            logger.log_text("Reconstructed Scheduler:", lr_scheduler)
